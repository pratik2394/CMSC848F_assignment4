...
class NeuralRadianceField(torch.nn.Module):
    def __init__(
        ...
        self.MultiLP = MLPWithInputSkips(cfg.n_layers_xyz, embedding_dim_xyz, 128 , embedding_dim_xyz, cfg.n_hidden_neurons_xyz, cfg.append_xyz)
        self.relu = torch.nn.ReLU()
        self.sigmoid = torch.nn.Sigmoid()
        self.linear1 = torch.nn.Linear(128, 4) #image size = 128

    def forward(self, ray_bundle):

        pose = self.harmonic_embedding_xyz(ray_bundle.sample_points.view(-1, 3))
        pred = self.linear1(self.MultiLP(pose, pose))

        output = {"density":None, "feature":None}
        output["density"] = self.relu(pred[:,0])
        output["feature"] = self.sigmoid(pred[:, 1:])
